<template>
  <div class="ph-root" v-if="file">
    ...
    <el-button-group class="mode-group">
      <el-button :disabled="!canPreview" :type="showPreview ? 'primary' : 'default'" @click="$emit('toggle-preview')">
        AI阅读
      </el-button>
      <el-button :disabled="!canPreview" :type="showText ? 'primary' : 'default'" @click="$emit('toggle-text')">
        文本模式
      </el-button>
      <el-button :disabled="!canPreview" :type="isTranslate ? 'primary' : 'default'" @click="$emit('translate')">
        翻译模式
      </el-button>
    </el-button-group>
    <!-- 刷新按钮移除，下载操作已移至文件名后 -->
